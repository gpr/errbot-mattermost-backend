...
class MattermostPerson(Person):
    ...
    def username(self) -> str:
        ...
        if 'username' not in user:
            log.error(
                "Can't find username for user with ID {}".format(self._userid))
    ...
    def fullname(self):
        ...
        if 'first_name' not in user and 'last_name' not in user:
            log.error(
                "No first or last name for user with ID {}".format(self._userid))
            return "<{}>".format(self._userid)
    ...
    def __eq__(self, other):
        if not isinstance(other, MattermostPerson):
            log.warning(
                'Tried to compare a MattermostPerson with a %s', type(other))
            return False

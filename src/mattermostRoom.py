...
class MattermostRoom(Room):
    ...
    def exists(self):
        channels = []
        channels.extend(self.driver.channels.get_channels_for_user(
            user_id='me', team_id=self.teamid))
    ...
    @property
    def occupants(self):
        member_count = self.driver.channels.get_channel_statistics(channel_id=self.id)[
            'member_count']
        members = {}
        user_page_limit = 200
    ...
    def invite(self, *args):
        ...
        for user in args:
            ...
            log.info('Inviting {} into {} ({})'.format(
                user, str(self), self.id))

            try:
                self.driver.channels.add_user(channel_id=self.id, options={
                                              'user_id': users[user]})

...
class MattermostRoomOccupant(RoomOccupant, MattermostPerson):
    ...
    def __init__(self, client, teamid, userid, channelid, bot):
        ...
        self._room = MattermostRoom(
            channelid=channelid, teamid=teamid, bot=bot)
    ...
    def __eq__(self, other):
        if not isinstance(other, RoomOccupant):
            log.warning(
                'tried to compare a MattermostRoomOccupant with a MattermostPerson {} vs {}'.format(self, other))
            return False
